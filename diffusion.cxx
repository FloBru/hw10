...
int main(){
  ...
  double dt = dx;
  // stabil bei dt <= (dx*dx)/(2*D)
  double t = 0;
  const int Na = 10; //10 Dateien 
  ...
  for(int i=1; i<=Na; i++)
  {
   for(int j=0; j<Nk; j++){
    ...
    //aus u0 wird u1 fuer den naechsten Schritt
    h = u0;
    u0 = u1;
    u1 = h;
